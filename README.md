...
Because of these patterns there is a high portion of code that can be considered as "boilerplate code" because it is either completely identical in all solution or can be derived after investigating the data model. `mdAL` aims at automatically generating this "boilerplate code" from a `mdAL` model file.
